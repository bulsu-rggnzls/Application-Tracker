...
import { Avatar, Badge, Button, Heading, IconButton, Input, Text } from '@/components/ui'
import { getTagStyle } from '@/utils/tagColors'
import CompanyLogo from './CompanyLogo'
import InterviewChecklist from './InterviewChecklist'
import getRelativeTime from '@/utils/getRelativeTime'
import formatSalary from '@/utils/formatSalary'
import useJobDetailForm from '../hooks/useJobDetailForm'
...
const inputCls = 'w-full px-3 py-2 text-sm text-slate-700 dark:text-slate-200 bg-surface border border-border rounded-lg placeholder-slate-400 focus-ring focus:border-brand transition-ui'
...
export default function JobDetailDrawer({ job, isOpen, onClose, onDelete, onStatusChange, onUpdate }) {
  const {
    ...
    activityLog,
    salaryLabel,
  } = useJobDetailForm({ job, isOpen, onStatusChange, onUpdate })
              ...
              <div className="flex gap-1 mt-5 bg-slate-100 dark:bg-slate-800 rounded-lg p-1">
                {TABS.map(({ id, label, icon: Icon }) => (
                  <button
                    ...
                    type="button"
                    onClick={() => setTab(id)}
                    className={`flex items-center justify-center gap-1.5 flex-1 px-2 py-2 text-xs font-semibold rounded-md transition-ui cursor-pointer ${
                      tab === id
                    ...
                    <div className="relative pl-7">
                      ...
                      {stages.map((stage, i) => {
                        ...
                        return (
                          ...
                          >
                            <div className={`absolute -left-[26px] top-0.5 w-[14px] h-[14px] rounded-full ring-2 flex items-center justify-center transition-ui ${nodeStyle} ${isFuture ? 'group-hover:bg-slate-400' : ''}`}>
