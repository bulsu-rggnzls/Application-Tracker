...
import { statusChip, statusDot, statusLabel } from '@/lib/status'
    ...
    return (
      <div
        ...
        className={`group bg-white dark:bg-slate-800/60 border ${borderClass} rounded-lg transition-[border-color,box-shadow] duration-150 cursor-pointer ${
          snapshot.isDragging
            ? 'shadow-xl rotate-2 !border-brand ring-2 ring-indigo-400/20 z-50'
            : 'hover:shadow-md'
        }`}
      >
          ...
          <div className="mt-2 flex items-center gap-1.5 flex-wrap min-w-0" onClick={e => e.stopPropagation()}>
            {mobile && onStatusChange && (
              <button
                ...
                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[11px] font-semibold transition-transform active:scale-95 cursor-pointer max-w-full ${statusChip(status)}`}
              >
